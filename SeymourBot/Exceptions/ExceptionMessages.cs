...
namespace SeymourBot.Exceptions
{
    ...
    class ExceptionMessages
    {
        public readonly static IDictionary<string, string> Messages = new Dictionary<string, string>()
        {
            ...
            {"0601",  $"Error creating new info command, ensure the DB path is {DBPaths.UserDB}" },
            {"0602", $"Error getting info command, ensure the DB path is {DBPaths.InfoDB}"},
            {"0603", $"Error getting info command list, ensure the DB path is {DBPaths.InfoDB}" },
            {"0604", $"Error getting timed events, ensure the DB path is {DBPaths.UserDB}" },
            {"0605", $"Error saving timed event, ensure the DB path is {DBPaths.UserDB}" },
            {"0606", $"Error archiving timed event, ensure the DB path is {DBPaths.UserDB}" },
            {"0607", $"Could not find the timed event to archive, ensure the DB path is {DBPaths.UserDB}" },
